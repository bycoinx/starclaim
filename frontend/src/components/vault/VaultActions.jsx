...
export default function VaultActions({ selectedStar, wallet }) {
  const walletMissing = !wallet;
  const starMissing = !selectedStar;
  ...
  return (
    <div className="rounded-[2rem] border border-white/10 bg-[#060a16]/88 p-6" role="region" aria-label="Legacy vault actions">
      <div className="flex items-center justify-between gap-4">
        ...
        {walletMissing && (
          <span className="text-xs uppercase tracking-[0.24em] text-sc-gold/70">Connect wallet to enable</span>
        )}
      </div>
      <div className="grid gap-3">
        <button disabled={walletMissing} className="flex items-center gap-3 p-3 rounded-xl bg-white/5 hover:bg-white/8 disabled:opacity-40 focus:outline-none focus:ring-2 focus:ring-indigo-400">
        ...
        <button disabled={walletMissing || starMissing} className="flex items-center gap-3 p-3 rounded-xl bg-white/5 hover:bg-white/8 disabled:opacity-40 focus:outline-none focus:ring-2 focus:ring-indigo-400">
