...
const fragmentShader = `
  ...
  void main() {
    float flameSpeed = time * 0.4;
    float noise = pnoise(vPosition * 0.4 + flameSpeed, vec3(10.0));
    float noise2 = pnoise(vPosition * 1.2 - flameSpeed * 0.5, vec3(10.0));
    
    vec3 color1 = vec3(1.0, 0.45, 0.05); // Vibrant Orange
    vec3 color2 = vec3(1.0, 0.9, 0.2);   // Brilliant Yellow
    vec3 color3 = vec3(0.8, 0.05, 0.0);  // Dark Solar Flare
    
    vec3 finalColor = mix(color1, color2, noise);
    finalColor = mix(finalColor, color3, noise2 * 0.6);
    
    // Pulse effect for intensity
    float pulse = 1.3 + 0.3 * sin(time * 0.5);
    float intensity = pulse + 0.5 * noise;
