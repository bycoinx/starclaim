...
function RotatingGalaxy({ onClaim, claimStatus, disabled }) {
  const ref = useRef()
  const isDisabled = disabled || claimStatus === 'pending'
  ...
  return (
    <group ref={ref}>
      ...
      <Html center position={[0, 0, 6.4]}>
        <button className={`claim-button ${claimStatus}`} onClick={onClaim} disabled={isDisabled}>
          {claimStatus === 'pending'
            ? 'Pending...'
            : claimStatus === 'success'
            ? 'Success!'
...
export default function ThreeScene({ onClaim, claimStatus, disabled }) {
  return (
    <div className="three-canvas">
      <Canvas camera={{ position: [0, 0, 20], fov: 45 }}>
        ...
        <RotatingGalaxy onClaim={onClaim} claimStatus={claimStatus} disabled={disabled} />
